...
function getNearestPointOnRoad(p, roadPoints) {
  ...
  for (let i = 0; i < roadPoints.length - 1; i++) {
    const v = roadPoints[i];
    const w = roadPoints[i + 1];
    const l2 = Math.pow(w.x - v.x, 2) + Math.pow(w.z - v.z, 2);
    if (l2 === 0) continue;
    let t = ((p.x - v.x) * (w.x - v.x) + (p.z - v.z) * (w.z - v.z)) / l2;
    t = Math.max(0, Math.min(1, t));
    const distSq = Math.pow(p.x - (v.x + t * (w.x - v.x)), 2) + Math.pow(p.z - (v.z + t * (w.z - v.z)), 2);
    if (distSq < minDistSq) {
      minDistSq = distSq;
      tParam = t;
      nearestIdx = i;
  ...
  if (nearestIdx < roadPoints.length - 1) {
    const v = roadPoints[nearestIdx];
    const w = roadPoints[nearestIdx + 1];
    return {
      point: new THREE.Vector3(v.x + tParam * (w.x - v.x), v.y + tParam * (w.y - v.y), v.z + tParam * (w.z - v.z)),
      distSq: minDistSq
    };
  }
  return null;
...
// ─── Three.js components ──────────────────────────────────────────────────────
const TerrainMesh = ({ gridData, heightScale, roadPoints, roadTexture, roadWidthWorld }) => {
  const { grid } = gridData;
  const res = grid.length;
  ...
  const geometry = useMemo(()=>{
    ...
    // roadWidthWorld is for 8m. We use half width for carving distance.
    const HALF_ROAD_WIDTH = roadWidthWorld / 2;
    const SHOULDER_WIDTH = roadWidthWorld; // 1.0x extra for transition
        ...
        if (roadPoints && roadPoints.length > 1) {
          ...
          if (nearest) {
            ...
            if (dist < HALF_ROAD_WIDTH) {
              ...
              const t = (dist - HALF_ROAD_WIDTH) / (SHOULDER_WIDTH - HALF_ROAD_WIDTH);
    ...
    return geo;
  }, [grid, res, minE, maxE, heightScale, roadPoints]);
...
const Terrain3D = ({ road, onClose }) => {
  ...
  const roadTexture = useMemo(() => {
    ...
    // Scale factors
    const metersToPixels = 2048 / gridWidthM;
    const roadWidthPixels = 8 * metersToPixels;
    const lineWidthPixels = 0.2 * metersToPixels;
    ...
    ctx.lineWidth = Math.max(1.5, lineWidthPixels); // Ensure it's at least visible
