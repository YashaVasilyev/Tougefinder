...
const TerrainMesh = ({ gridData, heightScale, displacementMap, roadTexture }) => {
  // Use a dense mesh but not quite 2048 (which crashes). 512 is plenty for displacement.
  const resolution = 512; 
  const geometry = useMemo(() => {
    const geo = new THREE.PlaneGeometry(100, 100, resolution - 1, resolution - 1);
    // Note: We don't modify vertex heights here. Displacement Map does it on GPU.
    return geo;
  }, [resolution]);
...
// Invisible component to compute road points for flyover
...
const Terrain3D = ({ road, onClose }) => {
  ...
  // Combined Texture: Maps + Displacement
  const textures = useMemo(() => {
    ...
    // 1. Displacement Map (Heights)
    const dCanvas = document.createElement('canvas');
    dCanvas.width = RES; dCanvas.height = RES;
    ...
    for (let y = 0; y < RES; y++) {
      for (let x = 0; x < RES; x++) {
        const u = x / (RES - 1);
        const v = 1 - (y / (RES - 1)); // Canvas Y is top-to-bottom
        const elev = sampleGrid(u, v);
        const norm = (elev - minE) / range;
    ...
    // 2. Road Texture (Colors)
    const cCanvas = document.createElement('canvas');
    cCanvas.width = RES; cCanvas.height = RES;
    ...
    // Carve into displacement map (flatten road)
    dCtx.strokeStyle = 'white'; // Use white to set height in displacement map? 
    // Wait, we need to draw the road height into dCanvas.
    // We'll use a special blending or just redraw the road with its heights.
    // For now, let's just use the asphalt for color.
    
    // Yellow Line
    cCtx.strokeStyle = '#facc15'; cCtx.lineWidth = Math.max(1.5, lineWidthPixels);
    ...
    const roadTex = new THREE.CanvasTexture(cCanvas);
    ...
    <div className={cn(
      ...
      <div className="absolute top-0 inset-x-0 p-4 flex items-center justify-between z-10 bg-gradient-to-b from-black/80 to-transparent">
        <div className="flex items-center gap-2">
          ...
          <span className="text-[10px] font-bold uppercase tracking-[0.2em] text-white/70">{flyoverActive ? '⬤ FLYOVER' : '3D Terrain Scan'}</span>
        </div>
      ...
      {loading ? (
        <div className="w-full h-full flex flex-col items-center justify-center gap-4">
          <Loader2 className="w-8 h-8 text-touge-500 animate-spin"/><span className="text-[10px] font-bold uppercase tracking-widest text-zinc-500">Processing Mesh...</span>
        </div>
      ) : (
            ...
            {flyoverActive && (
              <div className="p-3 bg-black/70 backdrop-blur border border-white/10 rounded-xl pointer-events-auto">
                <div className="flex items-center justify-between mb-2">
                  ...
                  <div className="flex items-center gap-3"><span className="text-[10px] font-mono text-touge-400">{displayKmh} km/h</span><span className="text-[10px] font-mono text-zinc-400">{(flyoverProgress*100).toFixed(1)}%</span></div>
                </div>
                ...
                <div className="flex items-center gap-2 mt-2">
                  ...
                  <input type="range" min={0.25} max={5} step={0.05} value={speedMultiplier} onChange={e=>setSpeedMultiplier(Number(e.target.value))} className="flex-1 h-1 appearance-none rounded-full cursor-pointer" style={{background:`linear-gradient(to right,#f97316 0%,#f97316 ${((speedMultiplier-0.25)/4.75)*100}%,#3f3f46 ${((speedMultiplier-0.25)/4.75)*100}%,#3f3f46 100%)`}}/><span className="text-[8px] text-zinc-600">5×</span>
                </div>
              </div>
            )}
            {!flyoverActive && (
              <div className="flex items-end gap-3 pointer-events-none">
                ...
                <div className="flex-1 p-3 bg-black/60 backdrop-blur border border-white/10 rounded-xl pointer-events-auto">
                  <div className="flex items-center justify-between mb-2">
                    <div className="flex items-center gap-1.5"><Mountain size={10} className="text-zinc-500"/><span className="text-[8px] font-bold uppercase tracking-widest text-zinc-500">Exaggeration</span></div>
                    <span className="text-[10px] font-mono text-touge-400">{heightScale}×</span>
                  </div>
                  <input type="range" min={1} max={100} step={1} value={heightScale} onChange={e=>setHeightScale(Number(e.target.value))} className="w-full h-1 appearance-none rounded-full cursor-pointer" style={{background:`linear-gradient(to right,#f97316 0%,#f97316 ${(heightScale-1)/99*100}%,#3f3f46 ${(heightScale-1)/99*100}%,#3f3f46 100%)`}}/>
                </div>
              </div>
            )}
          </div>
