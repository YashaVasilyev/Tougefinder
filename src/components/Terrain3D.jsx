...
const FlyoverBar = ({ progress, speedProfile, turnMarkers, onSeek }) => {
  ...
  return (
    <div ref={scrollRef} className="overflow-x-auto rounded-lg" style={{WebkitOverflowScrolling:'touch'}}>
      <div style={{width:1200, position:'relative', height:52, flexShrink:0}}>
        <div style={{position:'absolute',bottom:0,left:0,right:0,height:8,display:'flex',borderRadius:4,overflow:'hidden'}}>
          {segColors.map((c,i)=>(<div key={i} style={{flex:1,background:c,opacity:0.35}}/>))}
        </div>
        <div style={{position:'absolute',bottom:0,left:0,height:8,borderRadius:4,background:'white',opacity:0.9,transition:'none',width:`${progress*100}%`}}/>
        {turnMarkers.map((m,i)=>(
        ...
        <div style={{position:'absolute',inset:0,cursor:'pointer'}} onClick={e=>{const rect=e.currentTarget.getBoundingClientRect();onSeek((e.clientX-rect.left)/rect.width);}}/>
      </div>
    </div>
  );
};
...
const Terrain3D = ({ road, onClose }) => {
  ...
  useEffect(()=>{
    ...
    const tick=now=>{
      ...
      lastTimeRef.current=now; animRef.current=requestAnimationFrame(tick);
    };
  ...
  const localMult = speedProfile ? speedProfile[Math.min(Math.floor(flyoverProgress*(speedProfile.length-1)),speedProfile.length-1)] : 1;
  const displayKmh = Math.round(CAR_SPEED_MS*speedMultiplier*localMult*3.6);

  return (
    <div className={cn("fixed z-[2000] bg-zinc-950/95 backdrop-blur-3xl border border-white/10 shadow-2xl transition-all duration-500 overflow-hidden rounded-3xl", isFullscreen ? "inset-4" : "bottom-6 right-6 w-[480px] h-[480px]")}>
      <div className="absolute top-0 inset-x-0 p-4 flex items-center justify-between z-10 bg-gradient-to-b from-black/80 to-transparent">
        <div className="flex items-center gap-2"><div className="w-2 h-2 bg-touge-500 rounded-full animate-pulse"/><span className="text-[10px] font-bold uppercase tracking-[0.2em] text-white/70">{flyoverActive ? '⬤ FLYOVER' : '3D Terrain Scan'}</span></div>
        <div className="flex items-center gap-1">
          {!flyoverActive ? <button onClick={startFlyover} disabled={!roadPoints||!speedProfile} className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-[10px] font-bold uppercase tracking-wider bg-touge-500/20 border border-touge-500/40 text-touge-400 hover:bg-touge-500 hover:text-white transition-all disabled:opacity-30"><Video size={12}/> Flyover</button> : <><button onClick={()=>setFlyoverPlaying(p=>!p)} className="p-2 hover:bg-white/10 rounded-lg transition-colors text-white/60 hover:text-white">{flyoverPlaying ? <Pause size={14}/> : <Play size={14}/>}</button><button onClick={resetFlyover} className="p-2 hover:bg-white/10 rounded-lg transition-colors text-white/60 hover:text-white"><RotateCcw size={14}/></button></>}
          <button onClick={()=>setIsFullscreen(!isFullscreen)} className="p-2 hover:bg-white/10 rounded-lg transition-colors text-white/50 hover:text-white">{isFullscreen ? <Minimize2 size={16}/> : <Maximize2 size={16}/>}</button>
          ...
          <div className="absolute bottom-4 left-4 right-4 flex flex-col gap-2">
            {flyoverActive && (
              <div className="p-3 bg-black/70 backdrop-blur border border-white/10 rounded-xl pointer-events-auto">
                <div className="flex items-center justify-between mb-2"><span className="text-[8px] font-bold uppercase tracking-widest text-zinc-500">Route</span><div className="flex items-center gap-3"><span className="text-[10px] font-mono text-touge-400">{displayKmh} km/h</span><span className="text-[10px] font-mono text-zinc-400">{(flyoverProgress*100).toFixed(1)}%</span></div></div>
                <FlyoverBar progress={flyoverProgress} speedProfile={speedProfile} turnMarkers={turnMarkers} onSeek={handleSeek}/>
                <div className="flex items-center gap-2 mt-2"><span className="text-[8px] uppercase tracking-widest text-zinc-600 whitespace-nowrap w-12">{speedMultiplier.toFixed(2)}×</span><input type="range" min={0.25} max={5} step={0.05} value={speedMultiplier} onChange={e=>setSpeedMultiplier(Number(e.target.value))} className="flex-1 h-1 appearance-none rounded-full cursor-pointer" style={{background:`linear-gradient(to right,#f97316 0%,#f97316 ${((speedMultiplier-0.25)/4.75)*100}%,#3f3f46 ${((speedMultiplier-0.25)/4.75)*100}%,#3f3f46 100%)`}}/><span className="text-[8px] text-zinc-600">5×</span></div>
              </div>
            )}
            {!flyoverActive && (
              <div className="flex items-end gap-3 pointer-events-none">
                <div className="p-3 bg-black/60 backdrop-blur border border-white/10 rounded-xl pointer-events-none"><div className="text-[8px] font-bold uppercase tracking-widest text-zinc-500 mb-1">Telemetry</div><div className="text-xs font-mono text-touge-400">ALT: {Math.max(...gridData.grid.flat()).toFixed(0)}m MAX</div><div className="text-xs font-mono text-zinc-400">LAT: {gridData.minLat.toFixed(4)}</div></div>
                <div className="flex-1 p-3 bg-black/60 backdrop-blur border border-white/10 rounded-xl pointer-events-auto"><div className="flex items-center justify-between mb-2"><div className="flex items-center gap-1.5"><Mountain size={10} className="text-zinc-500"/><span className="text-[8px] font-bold uppercase tracking-widest text-zinc-500">Exaggeration</span></div><span className="text-[10px] font-mono text-touge-400">{heightScale}×</span></div><input type="range" min={1} max={100} step={1} value={heightScale} onChange={e=>setHeightScale(Number(e.target.value))} className="w-full h-1 appearance-none rounded-full cursor-pointer" style={{background:`linear-gradient(to right,#f97316 0%,#f97316 ${(heightScale-1)/99*100}%,#3f3f46 ${(heightScale-1)/99*100}%,#3f3f46 100%)`}}/></div>
              </div>
            )}
          </div>
        </div>
